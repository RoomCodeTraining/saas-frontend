...
@Component({
  selector: 'app-list-weight-loading',
  templateUrl: './list-weight-loading.component.html',
  styleUrls: ['./list-weight-loading.component.css']
})
export class ListWeightLoadingComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formGroupAdd = new FormGroup({
      ...
      bags: new FormControl('', [Validators.required, Validators.pattern('^-?[0-9]\\d*(\\.\\d{1,2})?$')]),
  ...
  getItems(){
    this.appService.getWeightLoading(this.user_logged.entity.id,this.current_page).subscribe((data: any) => {
  ...
  search(){
    this.SpinnerService.show();
    this.appService.getWeightLoadingSearch(this.user_logged.entity.id,this.current_page,this.information).subscribe((data: any) => {
      this.listItem = data.data;
  ...
  paginate(){
    if(this.information){
      ...
      this.SpinnerService.show();
      this.appService.getWeightLoadingPaginate(this.user_logged.entity.id,this.current_page).subscribe((data: any) => {
        this.listItem = data.data;
  ...
  save(){
    ...
    if(this.formGroupAdd.invalid){
      ...
      this.appService.addWeightLoading(requestData).subscribe(res => {
        this.message = res;
  ...
  update(){
    ...
    if(this.formGroupEdit.invalid){
      ...
      this.appService.updateWeightLoading(this.itemSelected.id,requestData).subscribe(res => {
        this.message = res;
        if(this.message.success == false){
          this.submit = false;
          this.error_message = this.message.message;
